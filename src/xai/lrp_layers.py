"""
This file is inspired by https://github.com/keio-smilab24/LRP-for-ResNet/tree/main?tab=License-1-ov-file#readme with amendments.
"""
import torch
...
def relevance_filter(r: torch.tensor, top_k_percent: float = 1.0) -> torch.tensor:
    ...
    Filter keeps k*100% of the largest tensor elements. Other tensor elements are set to
...
class RelevancePropagationMaxPool2d(nn.Module):
    ...
    def __init__(
        ...
        if mode == "avg":
            self.layer = torch.nn.AvgPool2d(kernel_size=layer.kernel_size, stride=layer.stride, padding=layer.padding, ceil_mode=layer.ceil_mode)
        elif mode == "max":
            self.layer = layer
